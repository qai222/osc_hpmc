...
from hpmc_2d import VERTICES_FOLDER
from hpmc_2d.polygon import render_plato

_cpu = hoomd.device.CPU()
assert hoomd.version.mpi_enabled  # comment out if not using mpi
...
class Sim2D:
    ...
    def run_compress(
        ...
        _cpu.notice(f"final area fraction set to: {final_volume_fraction}")
        ...
        _cpu.notice(f"compress # of timestep: {simulation.timestep}")
        ...
        if _rank == 0:
            self.snapshot_draw(snapshot, self.compress_png)
    ...
    def run(
        ...
        if _rank == 0:
            # these do not really benefit from parallelization
            self.pre_run()
        ...
        if _rank == 0:
            ...
            logger.critical("time cost: {:.2f}".format(ts2-ts1))
        #
    ...
    def state_draw(self, simulation: hoomd.Simulation, filename):
        ...
        #     scene = render_plato(self.vertices, snapshot.particles.position, snapshot.particles.orientation)
        #     scene.save(filename)
...
if __name__ == '__main__':
    SIM = Sim2D(vertices_file=f"{VERTICES_FOLDER}/vertices_tips_pn_2d.npy", shape_area=144.05592)
    SIM.run()
